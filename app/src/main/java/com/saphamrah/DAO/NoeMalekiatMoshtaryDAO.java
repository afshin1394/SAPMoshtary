...
import com.saphamrah.protos.CustomerOwnershipTypeGrpc;
import com.saphamrah.protos.CustomerOwnershipTypeReply;
import com.saphamrah.protos.CustomerOwnershipTypeReplyList;
import com.saphamrah.protos.CustomerOwnershipTypeRequest;
...
public class NoeMalekiatMoshtaryDAO
{
    ...
    public void fetchNoeMalekiatMoshtaryGrpc(final Context context, final String activityNameForLog, final RetrofitResponse retrofitResponse)
    {
        try {
...
//        serverIpModel.setServerIp("192.168.80.181");
            ...
            if (serverIpModel.getServerIp().trim().equals("") || serverIpModel.getPort().trim().equals("")) {
                ...
                PubFunc.Logger logger = new PubFunc().new Logger();
                logger.insertLogToDB(context, Constants.LOG_EXCEPTION(), message, NoeMalekiatMoshtaryDAO.class.getSimpleName(), activityNameForLog, "fetchNoeMalekiatMoshtaryGrpc", "");
                retrofitResponse.onFailed(Constants.RETROFIT_HTTP_ERROR(), message);
            } else {

                CompositeDisposable compositeDisposable = new CompositeDisposable();
                ManagedChannel managedChannel = GrpcChannel.channel(serverIpModel);
                CustomerOwnershipTypeGrpc.CustomerOwnershipTypeBlockingStub customerOwnershipTypeBlockingStub = CustomerOwnershipTypeGrpc.newBlockingStub(managedChannel);
                CustomerOwnershipTypeRequest customerOwnershipTypeRequest = CustomerOwnershipTypeRequest.newBuilder().build();
                Callable<CustomerOwnershipTypeReplyList> customerOwnershipTypeReplyListCallable = () -> customerOwnershipTypeBlockingStub.getCustomerOwnershipType(customerOwnershipTypeRequest);
                RxAsync.makeObservable(customerOwnershipTypeReplyListCallable)
                        .map(customerOwnershipTypes  ->  {
                            ArrayList<NoeMalekiatMoshtaryModel> models = new ArrayList<>();
                            for (CustomerOwnershipTypeReply reply : customerOwnershipTypes.getCustomerOwnershipTypesList()) {

                                NoeMalekiatMoshtaryModel model = new NoeMalekiatMoshtaryModel();

                                model.setCcNoeMalekiatMoshtary(reply.getCustomerOwnershipTypeID());
                                model.setNameNoeMalekiatMoshtary(reply.getCustomerOwnershipName());
                        ...
                        .subscribe(new Observer<ArrayList<NoeMalekiatMoshtaryModel>>() {
                            @Override
                            ...
                            @Override
                            public void onNext(@NonNull ArrayList<NoeMalekiatMoshtaryModel> noeMalekiatMoshtaryModels) {
                                retrofitResponse.onSuccess(noeMalekiatMoshtaryModels);
                            }
            ...
            PubFunc.Logger logger = new PubFunc().new Logger();
            logger.insertLogToDB(context, Constants.LOG_EXCEPTION(), exception.getMessage(), NoeMalekiatMoshtaryDAO.class.getSimpleName(), activityNameForLog, "fetchNoeMalekiatMoshtaryGrpc", "");
