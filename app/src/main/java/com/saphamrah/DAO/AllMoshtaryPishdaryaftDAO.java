...
import com.saphamrah.protos.UpFrontReceiptCustomerGrpc;
import com.saphamrah.protos.UpFrontReceiptCustomerReply;
import com.saphamrah.protos.UpFrontReceiptCustomerReplyList;
import com.saphamrah.protos.UpFrontReceiptCustomerRequest;
    ...
    public void fetchAllMoshtaryforoshandehGrpc(final Context context, final String activityNameForLog, final String ccForoshandeh, final RetrofitResponse retrofitResponse)
    {
        try {
            ...
            if (serverIpModel.getServerIp().trim().equals("") || serverIpModel.getPort().trim().equals(""))
            {
                ...
                UpFrontReceiptCustomerGrpc.UpFrontReceiptCustomerBlockingStub blockingStub = UpFrontReceiptCustomerGrpc.newBlockingStub(managedChannel);
                UpFrontReceiptCustomerRequest request = UpFrontReceiptCustomerRequest.newBuilder().setSalesManID(ccForoshandeh).build();

                Callable<UpFrontReceiptCustomerReplyList> replyListCallable  = () -> blockingStub.getUpFrontReceiptCustomer(request);
                RxAsync.makeObservable(replyListCallable)

                        .map(replyList -> {
                            ArrayList<AllMoshtaryPishdaryaftModel> models = new ArrayList<>();
                            for (UpFrontReceiptCustomerReply reply : replyList.getUpFrontReceiptCustomersList()) {
                                AllMoshtaryPishdaryaftModel model = new AllMoshtaryPishdaryaftModel();
                                ...
                                model.setCcMasir(reply.getRoutID());
                                ...
                                model.setCcNoeMoshtary(reply.getCustomerID());
                        ...
                        .subscribe(new Observer<ArrayList<AllMoshtaryPishdaryaftModel>>() {
                            @Override
                            ...
                            @Override
                            public void onNext(@NonNull ArrayList<AllMoshtaryPishdaryaftModel> models) {
