...
import com.saphamrah.protos.PersonsBinGrpc;
import com.saphamrah.protos.PersonsBinReply;
import com.saphamrah.protos.PersonsBinReplyList;
import com.saphamrah.protos.PersonsBinRequest;
...
public class AnbarakAfradDAO 
{
    ...
    public void fetchAnbarakAfradGrpc(final Context context, final String activityNameForLog,final String ccAfradForoshandehMamorPakhsh, final RetrofitResponse retrofitResponse)
    {
        try {
...
//        serverIpModel.setServerIp("192.168.80.181");
            ...
            if (serverIpModel.getServerIp().trim().equals("") || serverIpModel.getPort().trim().equals("")) {
                ...
                PubFunc.Logger logger = new PubFunc().new Logger();
                logger.insertLogToDB(context, Constants.LOG_EXCEPTION(), message, AnbarakAfradDAO.class.getSimpleName(), activityNameForLog, "fetchAnbarakAfradGrpc", "");
                retrofitResponse.onFailed(Constants.HTTP_WRONG_ENDPOINT(), message);
            } else {

                CompositeDisposable compositeDisposable = new CompositeDisposable();
                ManagedChannel managedChannel = GrpcChannel.channel(serverIpModel);
                PersonsBinGrpc.PersonsBinBlockingStub personsBinBlockingStub = PersonsBinGrpc.newBlockingStub(managedChannel);
                PersonsBinRequest personsBinRequest = PersonsBinRequest.newBuilder().setSellerPersonsDistributorID(ccAfradForoshandehMamorPakhsh).build();
                Callable<PersonsBinReplyList> personsBinReplyListCallable = () -> personsBinBlockingStub.getPersonsBin(personsBinRequest);
                RxAsync.makeObservable(personsBinReplyListCallable)
                        .map(personsBinReplyList -> {
                            ArrayList<AnbarakAfradModel> anbarakAfradModels = new ArrayList<>();
                            for (PersonsBinReply personsBinReply : personsBinReplyList.getPersonsBinsList()) {

                                AnbarakAfradModel anbarakAfradModel = new AnbarakAfradModel();

                                anbarakAfradModel.setCcAnbarak(personsBinReply.getBinID());
                                anbarakAfradModel.setCcAfradForoshandeh(personsBinReply.getSellerPersonsID());
                                anbarakAfradModel.setCcAfradMamorPakhshJaygozin(personsBinReply.getAlternativeDistributorPersonsID());
                                anbarakAfradModel.setNameAnbarak(personsBinReply.getBinName());
                                anbarakAfradModel.setCcAfradRanandehJaygozin(personsBinReply.getAlternativeDriverPersonsID());
                                anbarakAfradModel.setCcAfradForoshandehJaygozin(personsBinReply.getSellerPersonsID());
                                anbarakAfradModel.setCcAfradMamorPakhsh(personsBinReply.getDistributorPersonsID());
                                anbarakAfradModel.setCcAfradRanandeh(personsBinReply.getDriverPersonsID());
                                anbarakAfradModel.setCcNoeForoshandeh(personsBinReply.getSellerTypeID());
                        ...
                        .subscribe(new Observer<ArrayList<AnbarakAfradModel>>() {
                            @Override
                            ...
                            @Override
                            public void onNext(@NonNull ArrayList<AnbarakAfradModel> anbarakAfradModels) {
                                retrofitResponse.onSuccess(anbarakAfradModels);
                            }
                            ...
                            public void onError(@NonNull Throwable e) {
                                e.printStackTrace();
                                PubFunc.Logger logger = new PubFunc().new Logger();
                                logger.insertLogToDB(context, Constants.LOG_EXCEPTION(), e.toString(), AnbarakAfradDAO.class.getSimpleName(), activityNameForLog, "fetchAnbarakAfradGrpc", "CatchException");
            ...
            exception.printStackTrace();
            PubFunc.Logger logger = new PubFunc().new Logger();
            logger.insertLogToDB(context, Constants.LOG_EXCEPTION(), exception.toString(), AnbarakAfradDAO.class.getSimpleName(), activityNameForLog, "fetchAnbarakAfradGrpc", "CatchException");
