...
public class ParameterChildDAO
{
    ...
    public void fetchParameterChildGrpc(final Context context, final String activityNameForLog, String noeTitrSatr, String ccMarkazSazmanForosh, String ccMarkazAnbar, String dateProgram, final RetrofitResponse retrofitResponse) {
        try {
            ...
            if (serverIpModel.getServerIp().trim().equals("") || serverIpModel.getPort().trim().equals("")) {
                ...
                ParameterChildGrpc.ParameterChildBlockingStub blockingStub = ParameterChildGrpc.newBlockingStub(managedChannel);
                ParameterChildRequest request = ParameterChildRequest.newBuilder().setRowTitleType(noeTitrSatr).setSellOrganizationCenterID(ccMarkazSazmanForosh).setStoreCenterID(ccMarkazAnbar).setDateProgram(dateProgram).build();

                Callable<ParameterChildReplyList> replyListCallable = () -> blockingStub.getParameterChild(request);
                RxAsync.makeObservable(replyListCallable)

                        .map(replyList -> {
                            ArrayList<ParameterChildModel> models = new ArrayList<>();
                            for (ParameterChildReply reply : replyList.getParameterChildsList()) {
                                ParameterChildModel model = new ParameterChildModel();

                                model.setCcParameterChild(reply.getParameterChildID());
                                model.setCcParameter(reply.getParameterID());
                                model.setName(reply.getName());
                                model.setValue(reply.getValue());
                                model.setTxt(reply.getTxt());
                                model.setCodeSort(reply.getSortCode());
                                models.add(model);
                            }
                        ...
                        .subscribe(new Observer<ArrayList<ParameterChildModel>>() {
                            @Override
                            ...
                            @Override
                            public void onNext(@NonNull ArrayList<ParameterChildModel> models) {
