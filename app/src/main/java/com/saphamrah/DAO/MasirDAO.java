...
import com.saphamrah.protos.RouteGrpc;
import com.saphamrah.protos.RouteReply;
import com.saphamrah.protos.RouteReplyList;
import com.saphamrah.protos.RouteRequest;
...
public class MasirDAO
{
    ...
    public void fetchAllMasirGrpc(final Context context, final String activityNameForLog,final String ccForoshandeh,final String ccMarkazForosh,final String azTarikh,final String taTarikh,final String codeNoe, final RetrofitResponse retrofitResponse) {
        try {
            ServerIpModel serverIpModel = new PubFunc().new NetworkUtils().getServerFromShared(context);
            if (serverIpModel.getServerIp().trim().equals("") || serverIpModel.getPort().trim().equals("")) {
                String message = "can't find server";
                PubFunc.Logger logger = new PubFunc().new Logger();
                logger.insertLogToDB(context, Constants.LOG_EXCEPTION(), message, MasirDAO.class.getSimpleName(), activityNameForLog, "fetchAllMasirGrpc", "");
                retrofitResponse.onFailed(Constants.RETROFIT_HTTP_ERROR(), message);
            } else {

                CompositeDisposable compositeDisposable = new CompositeDisposable();
                ManagedChannel managedChannel = GrpcChannel.channel(serverIpModel);
                RouteGrpc.RouteBlockingStub blockingStub = RouteGrpc.newBlockingStub(managedChannel);
                RouteRequest request = RouteRequest.newBuilder().setSellerID(ccForoshandeh).setSellCenterID(ccMarkazForosh).setFromDate(azTarikh).setToDate(taTarikh).setTypeCode(codeNoe).build();

                Callable<RouteReplyList> replyListCallable = () -> blockingStub.getRoute(request);
                RxAsync.makeObservable(replyListCallable)

                        .map(replyList -> {
                            ArrayList<MasirModel> models = new ArrayList<>();
                            for (RouteReply reply : replyList.getRoutesList()) {
                                MasirModel model = new MasirModel();

                                model.setCcMasir(reply.getRouteID());
                                model.setCcForoshandeh(reply.getSellerID());
                                model.setNameMasir(reply.getRouteName());
                                model.setToorVisit(reply.getVisitTour());
                                model.setCcMasirRoozVisit(reply.getVisitTodayRouteID());
                                models.add(model);
                            }
                        ...
                        .subscribe(new Observer<ArrayList<MasirModel>>() {
                            ...
                            @Override
                            public void onNext(@NonNull ArrayList<MasirModel> models) {
