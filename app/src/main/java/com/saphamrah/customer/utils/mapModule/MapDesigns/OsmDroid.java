package com.saphamrah.customer.utils.mapModule.MapDesigns;
...
import com.saphamrah.customer.utils.mapModule.Models.CirclePolygonModel;
import com.saphamrah.customer.utils.mapModule.Models.MapLineModel;
import com.saphamrah.customer.utils.mapModule.Models.MapObjectModel;
import com.saphamrah.customer.utils.mapModule.Models.PolygonModel;
...
public class OsmDroid extends MapInstructor {
     ...
     * @param showTarh
     */
    public OsmDroid(Context context, MapView mapView, boolean showTarh, MapType mapType) {
        super(context);

        this.context = context;
        this.mapView = mapView;
        mapView.setMultiTouchControls(true);
        if (mapType.equals(MapType.OsmDroid))
        {
            mapView.getTileProvider().clearTileCache();
            mapView.setTileSource(TileSourceFactory.MAPNIK);
        }
        if (mapType.equals(MapType.Offline))
        {
            mapView.getTileProvider().clearTileCache();
            mapView.setMinZoomLevel(15d);
            mapView.setMaxZoomLevel(22d);
            ITileSource tileSource = new XYTileSource(Constants.OFFLINE_MAP_TILES,Constants.OFFLINE_MIN_ZOOM , Constants.OFFLINE_MAX_ZOOM, 256, ".png", new String[0]);
    ...
    public void onMapContentClickListener(String groupMarkers, IMapClickEvents iMapClickEvents)
    {
        ...
        mapView.getOverlays().add(new MapEventsOverlay(new MapEventsReceiver() {
            ...
            public boolean singleTapConfirmedHelper(GeoPoint p) {
                ...
                if (i == -1)
                    iMapClickEvents.onOtherItemsClick(p);
                else
